...
export const viewEventController = async (req, res) => {
    try {
        const events = await Create_Event.find({ Delete: false });
...
export const deleteEventController = async (req, res) => {
    ...
    try {
      ...
      const categories = await Create_Event.find({ Delete: false });
  ...
  export const getCompletedEventsController = async (req, res) => {
    try {
        const completedEvents = await Create_Event.find({ Status: 'completed',Delete:'false' });
...
export const getEventCount = async (req, res) => {
    try {
      ...
      const categoryCount = await Create_Event.countDocuments({ Delete: false });
  ...
  export const getCompleteCount = async (req, res) => {
    try {
      // Count only the Event_category records where delete is false
      const categoryCount = await Create_Event.countDocuments({Status: 'completed', Delete: false });
      res.json( categoryCount );
    } catch (err) {
      console.error('Error counting Event categories:', err);
      res.status(500).json({ error: 'Server error' });
