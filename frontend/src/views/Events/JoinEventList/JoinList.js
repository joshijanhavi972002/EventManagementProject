...
export default function CategoryListDrawer(props) {
    ...
    const handleUpdate = async () => {
        try {
            await eventMemberValidationSchema.validate(updatedData, { abortEarly: false });
            const response = await updateData(`http://localhost:3001/AddEventMemberapi/UpdateEventMember/${data}`, updatedData);
            ...
            {['right'].map((anchor) => (
                <React.Fragment key={anchor}>
                    ...
                    >
                        ...
                        {mode === "edit" ? <form encType="multipart/form-data" onSubmit={handleUpdate} method='PUT' >
                            <Grid >
                                <Grid item xs={12}>
                                    <TextField
                                        id="Event_Name"
                                ...
                                {/* <Grid item xs={12}>
                                    <TextField
                                        ...
                                        onChange={handleInputChange}     
                                        style={{ marginBottom: '15px', maxWidth: '300px' }}
                                        value={updatedData?.Attendee_Status || ''}
